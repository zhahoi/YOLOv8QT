...
bool YOLOv8::init(const DetectorConfig& config)
{
    ...
    std::string param_file = m_config.model_path + ".param";
    std::string bin_file   = m_config.model_path + ".bin";

    std::cout << "param_file: " << param_file;
    std::cout << "bin_file: " << bin_file;

    yolo.load_param(param_file.c_str());
    yolo.load_model(bin_file.c_str());
...
int YOLOv8::detect(const cv::Mat &rgb, std::vector<Object> &objects)
{
    ...
    if (w > h)
    {
        scale = (float)m_config.target_size / w;
        w = m_config.target_size;
        h = h * scale;
    }
    else
    {
        scale = (float)m_config.target_size / h;
        h = m_config.target_size;
        w = w * scale;
    }
    ...
    ncnn::Extractor ex = this->yolo.create_extractor();
    ...
    generate_proposals(out, strides, in_pad, m_config.prob_threshold, proposals);
    ...
    nms_sorted_bboxes(proposals, picked, m_config.nms_threshold);
